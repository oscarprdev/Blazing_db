	...
	return (
		<div className="absolute bottom-5 flex flex-col gap-2 w-full max-w-[36rem] lg:max-w-[40rem] xl:max-w-[48rem]">
			...
			<div className="w-full">
				<form
					action={handleSubmit}
					className="flex items-center md:gap-5 lg:gap-6 w-full rounded-xl p-2 border border-dark3 bg-dark2 hover:bg-dark1 duration-200">
...
function FormAiTextarea({
	...
	return (
		<Textarea
			...
			rows={1}
			className="m-0 pl-2 py-1 resize-none bg-transparent border border-dark2 text-token-text-light2 overflow-hidden h-[34px]"
		/>
	);
}
...
function FormAiSubmitButton({ prompt }: { prompt: string }) {
	...
	return (
		<Button
			type="submit"
			className="p-1.5 px-5 min-w-[100px] grid place-items-center mt-auto"
			disabled={isDisabled}>
			{pending ? <IconLoader2 className="animate-spin" size={20} /> : 'Generate'}
		</Button>
	);
}
