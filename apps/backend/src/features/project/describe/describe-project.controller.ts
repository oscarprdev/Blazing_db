import { provideDescribeProjectPostgreUsecaseSingleton } from './describe-project.index';
import { Database } from '@/db';
import { ProjectType } from '@/types';

export class DescribeProjectController {
	...
	async serveHandler(projectId: string) {
		...
		switch (type) {
			case ProjectType.postgreSQL:
				return provideDescribeProjectPostgreUsecaseSingleton(this.db);
			default:
